<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="失主姓名" prop="shizhuxingming">
          <el-input v-model="ruleForm.shizhuxingming" 
              placeholder="失主姓名" clearable  :readonly="ro.shizhuxingming"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="失主姓名" prop="shizhuxingming">
              <el-input v-model="ruleForm.shizhuxingming" 
                placeholder="失主姓名" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="信用积分" prop="xinyongjifen">
          <el-input v-model="ruleForm.xinyongjifen" 
              placeholder="信用积分" clearable  :readonly="ro.xinyongjifen"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="信用积分" prop="xinyongjifen">
              <el-input v-model="ruleForm.xinyongjifen" 
                placeholder="信用积分" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    yonghumingChange () {
      this.$http({
        url: `follow/shizhu/yonghuming?columnValue=`+ this.ruleForm.yonghuming,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.shizhuxingming){
                this.ruleForm.shizhuxingming = data.data.shizhuxingming
              }
        } else {
          this.$message.error(data.msg);
        }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shizhuxinyong/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shizhuxinyongCrossAddOrUpdateFlag = false;
                  this.parent.search();
