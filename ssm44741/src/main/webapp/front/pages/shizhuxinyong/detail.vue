	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					<view class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>{{detail.shizhuxingming}}</view>
                				                				...
                				                				<view class="detail-list-item" :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","margin":"0 0 20rpx 0","borderColor":"#fff","backgroundColor":"#fff","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(255, 153, 0, 1)","textAlign":"left"}'>信用积分：</view>
					<view class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>{{detail.xinyongjifen}}</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','shizhuxinyong');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','shizhuxinyong');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('shizhuxinyong', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussshizhuxinyong/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('shizhuxinyong', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('shizhuxinyong', this.detail);
				this.$utils.msg('审核成功');
