<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(255, 153, 0, 1)","textAlign":"left"}' class="title">投诉日期</view>
				<picker mode="date" :value="ruleForm.tousuriqi" @change="tousuriqiChange">
					<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(255, 153, 0, 1)","borderRadius":"16rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{ruleForm.tousuriqi?ruleForm.tousuriqi:"请选择投诉日期"}}</view>
				</picker>
			</view>
			...
			<!-- 否 -->
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(255, 153, 0, 1)","textAlign":"left"}' class="title">投诉内容</view>
				<textarea :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(255, 153, 0, 1)","borderRadius":"16rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"300rpx"}' v-model="ruleForm.tousuneirong" placeholder="投诉内容"></textarea>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			this.user = res.data;
						...
						// ss读取
												...
												this.ruleForm.shizhuxingming = this.user.shizhuxingming
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`shizhutousu`, this.ruleForm.id);
		...
		methods: {
			...
			// 多级联动参数
																																	...
																																	tousuriqiChange(e) {
				this.ruleForm.tousuriqi = e.target.value;
				this.$forceUpdate();
			},
			...
			async onSubmitTap() {
																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																if(this.ruleForm.id){
					await this.$api.update(`shizhutousu`, this.ruleForm);
				}else{
					await this.$api.add(`shizhutousu`, this.ruleForm);
				}
